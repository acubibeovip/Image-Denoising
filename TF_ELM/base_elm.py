...
class TF_ELM():
    ...
    def train_test(self, diff, vel, x_input, y_input, iInput, bInput):
        np.set_printoptions(threshold=np.inf)
        # self._weight = np.load('w.npy')
        # self._bias = np.load('bias.npy')
        # print((self._weight))
        # print((self._bias))
        start = time.perf_counter()
        phi1 = self._activation(tf.add(tf.matmul(self._x, self._weight), self._bias))
        ...
        casepath = os.path.join("c:/Users/sonli/OneDrive/Desktop/Solving-PDE-With-NN", 'information.txt')
        myfile = open(casepath, 'w+')
        myfile.write("\tResidual: " + str(res_matrix) + "\n"*100)
        myfile.write("\tICs: " + str(ICs) + "\n"*100)
        myfile.write("\tBCs: " + str(BCs) + "\n"*100)
        
        H_matrix = tf.concat([res_matrix, ICs, BCs], axis=0)
        h_inv = tf.linalg.pinv(H_matrix)
        #print("H_matrix: ", self._sess.run(H_matrix))
        
        myfile.write("\h_inv tensorflow: " + str(self._sess.run(h_inv)) + "\n"*100)
        self._beta = tf.matmul(h_inv, self._y)
        
        self._beta = self._sess.run(self._beta, feed_dict={self._y: y_input})
        #print("beta: ", self._beta)
        myfile.write("\tbeta: " + str(self._beta) + "\n"*100)
        end = time.perf_counter()
        print("Hidden nodes:", self.hiddenNodes)
        print("Training time:", str(timedelta(seconds=(end - start))))
    ...
    def N(self, x, W, beta): 
        wt = np.tanh(np.add(np.dot(x, W), (self._sess.run(self._bias))))
        return ((np.dot(wt, beta)))    
    
    def eml(self, x, t):
        em = np.zeros_like(x)
        for i,v in enumerate(x):
            em[i] =  self.N(np.array([v,t]), (self._sess.run(self._weight)), self._beta)
        return em
